...
fig_dir = os.path.join('/home/disk/p/wperkins/ipynb/ens_pred/new_standardized_figs',
...
if plot_lim_noise_eofs:
    ...
    for var_key in var_names:
        ...
        noise_eofs[var_key] = dobj_eofs @ state_eofs @ Q_evect
...
for comparison_exp, compare_avg_key in fcast_against.items():
    ...
    comp_state = lutils.LIMState(comp_dobjs)
...
if do_perfect_fcast:
    ...
    if not fcast_against_only:
        ...
        fcast_1yr_basis = state.proj_data_into_orig_basis(fcast_1yr)
...
if do_ens_fcast:
    ...
    if not fcast_against_only:
        ...
        ens_output_full = state.proj_data_into_orig_basis(ens_output)
